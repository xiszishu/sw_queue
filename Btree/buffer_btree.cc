...
// void replace(BTreeNode *t,int k2)
// {
//     int i = 0;
//     while (i < n && k1 > t->keys[i])
//         i++;
...
void BTree::insert(int k)
{
	...
	if (root == NULL)
	{
      ...
      rt_mem->write_literal(&k, sizeof(int), &root->keys[0]);
      //rt_mem->do_transfer();
      //rt_mem->write_literal(&root->keys[0], sizeof(int), &a[k1]);
	}
...
void BTreeNode::insertNonFull(int k)
{
    ...
    //std::cout<<"insert not full key:"<<k<<std::endl;
	int i = n-1;
  int now=-1;
    ...
    //std::cout<<"0"<<std::endl;
      if (i>=0) now=keys[i];
		...
		//keys[i+1] = k;
    rt_mem->write_literal(&k, sizeof(int), &keys[i+1]);
    //printf("insert address:%p\n",&keys[i+1]);
		n = n+1;
	}
